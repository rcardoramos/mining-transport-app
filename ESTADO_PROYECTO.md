...
| **Fase 4** | **Captura de Pasajeros** | 🟢 **Completado** | Registro de embarque mediante escaneo QR y DNI manual delegados en el módulo modular lib/features/passenger/ y sus casos de uso. |
...
| **Fase 7** | **Control de Aforo** | 🟢 **Completado** | Contador visual dinámico de aforo a bordo en tiempo real y validación de capacidad máxima del bus (RN-AFO-02-01) implementados mediante ValidateOccupancyUseCase modular. |
...
* **Envío de Campos Puesto y Unidad**: Se agregaron los campos `puesto` y `unidad` en los flujos de validación y registro de pasajeros, garantizando que el backend de producción reciba esta información obligatoria de los colaboradores de Miski Mayo.
* **Manejo Resiliente del JSON de Aforo**: Se adaptó el parseo de la respuesta en `home_dashboard_remote_data_source_impl.dart` para leer de forma resiliente la capacidad y cantidad de pasajeros ocupados del backend (`Capacidad` y `Ocupados` dentro del objeto `Data`), construyendo el modelo de viaje de retorno de forma limpia.
* **Filtro de Formatos de Escaneo (onDetect)**: Se implementó un filtro de formatos a nivel de callback en `qr_scanner_page.dart` que acepta únicamente `BarcodeFormat.pdf417` (DNI) y `BarcodeFormat.qrCode` (Fotocheck corporativo), ignorando de forma nativa e inmediata el código vertical `Code 39` al reverso del DNI para evitar lecturas de texto corruptas.
* **Limpieza de Generación de Código**: Se removieron los archivos de parte `.g.dart` obsoletos de varios modelos con deserializadores manuales personalizados, resolviendo conflictos de compilación en el compilador de Dart.
